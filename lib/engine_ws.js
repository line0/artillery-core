...
WSEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
  ...
  return function scenario(initialContext, callback) {
    ...
    let steps = _.flatten([
      ...
      tasks,
      (ctx, next) => {
        if (!scenarioSpec.beforeSocketClose)
          return next(null, ctx)
        const processFunc = config.processor[scenarioSpec.beforeSocketClose];
        if (processFunc) {
          processFunc(scenarioSpec, config, ctx, ee, () => next(null, ctx));
        } else {
          const msg = `Could not find a matching beforeSocketClose function with name '${scenarioSpec.beforeSocketClose}' in processor script.`;
          debug(msg);
          ee.emit('error', msg);
          next(msg, ctx);
        }
